...
Route::group(['domain' => domain()], function () {
    ...
    Route::name('backend.')
        ...
        Route::name('categories.')->prefix('categories')->group(function () {
            ...
            Route::get('{category}')->name('edit')->uses('CategoriesController@form')->where('category', '[a-z0-9-]+');
            Route::put('{category}')->name('update')->uses('CategoriesController@update')->where('category', '[a-z0-9-]+');
            Route::get('{category}/logs')->name('logs')->uses('CategoriesController@logs')->where('category', '[a-z0-9-]+');
            Route::delete('{category}')->name('delete')->uses('CategoriesController@delete')->where('category', '[a-z0-9-]+');
        });
